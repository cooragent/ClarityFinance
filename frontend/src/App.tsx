...
import {
  ...
  Flame, LayoutDashboard, LineChart as LineIcon, ListFilter, Menu, Plus,
  Radar, RefreshCw, Search, Send, Star, Trash2, UserRoundSearch, WalletCards, X,
} from "lucide-react";
...
const nav: { id: Page; label: string; icon: typeof Bot; group: string }[] = [
  { id: "vibe", label: "Vibe 量化内核", icon: Bot, group: "工作台" },
  { id: "hotspots", label: "今日热点", icon: Flame, group: "工作台" },
  { id: "dashboard", label: "决策仪表盘", icon: LayoutDashboard, group: "工作台" },
  { id: "analyze", label: "股票分析", icon: Search, group: "研究" },
  { id: "evolve", label: "自演进组合", icon: BrainCircuit, group: "研究" },
  { id: "backtest", label: "策略回测", icon: LineIcon, group: "研究" },
  { id: "holdings", label: "我的持仓", icon: WalletCards, group: "资产" },
  { id: "track", label: "持仓跟踪", icon: UserRoundSearch, group: "资产" },
  { id: "screen", label: "股票筛选", icon: ListFilter, group: "智能体" },
  { id: "ask", label: "智能问答", icon: Radar, group: "智能体" },
];

const pageMeta: Record<Page, [string, string]> = {
  vibe: ["Vibe 量化内核", "把复杂策略研究交给独立运行的 Vibe-Trading"],
  hotspots: ["今日热点", "梳理全球 10 大事件，并搜索关联股票"],
  dashboard: ["决策仪表盘", "扫描市场，生成今日关注清单"],
  analyze: ["股票分析", "多智能体基本面与交易研究"],
  evolve: ["自演进组合", "Follow 公开组合，持续回测并迭代参数"],
  holdings: ["我的持仓", "统一管理关注标的与真实仓位"],
  backtest: ["策略回测", "验证均线策略的收益、风险与交易记录"],
  track: ["持仓跟踪", "研究明星投资者和基金的公开持仓"],
...
function Markdown({ value }: { value?: string }) {
  return value ? <div className="markdown"><ReactMarkdown>{value}</ReactMarkdown></div> : null;
}
...
function AddHolding({ ticker, name = "" }: { ticker: string; name?: string }) {
  ...
  async function add() {
    ...
    try { await post("/api/v1/holdings", { ticker, name, quantity: 0, avg_cost: 0 }); setDone(true); }
...
function HotspotsPage() {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const load = () => { setBusy(true); setError(""); get<Json>("/api/v1/hotspots").then(setData).catch((e) => setError(e.message)).finally(() => setBusy(false)); };
  useEffect(load, []);
  ...
  return <div className="stack">
    <Card title={`${data.date || today} · 全球热点`} action={<Button secondary busy={busy} onClick={load}><RefreshCw size={14} />刷新</Button>}>
      <div className="hotspot-list">{(data.hotspots || []).map((item: Json, index: number) => <button key={item.link || index} className={`hotspot ${selected === item.title ? "selected" : ""}`} onClick={() => search(item.title)}>
        <strong>{String(index + 1).padStart(2, "0")}</strong><span><b>{item.title}</b><small>{item.source} · {item.published}</small></span><ChevronRight size={18} />
      </button>)}</div><ErrorBox error={error} />
    </Card>
    {selected && <Card title="关联股票"><p className="muted">{selected}</p><div className="stock-grid">{(related.stocks || []).map((stock: Json) => <article className="stock-card" key={stock.symbol}><div><strong>{stock.symbol}</strong><span>{stock.name}</span></div><small>{stock.market} · {stock.relation}</small><AddHolding ticker={stock.symbol} name={stock.name} /></article>)}</div>{!busy && !(related.stocks || []).length && <div className="empty">暂未找到明确关联的上市公司</div>}</Card>}
  </div>;
}
...
function EvolvePage() {
  ...
  async function follow(id: string) {
    setBusy(id); setError("");
    try { setResult(await post<Json>(`/api/v1/portfolio/featured/${id}/follow`, { profile_prefix: form.profile, risk: form.risk, portfolio_size: form.portfolio_size, target_return_pct: form.target_return_pct, max_drawdown_pct: form.max_drawdown_pct, trading_cost_pct: form.trading_cost_pct, years: Math.max(1, Math.round((Date.parse(form.end) - Date.parse(form.start)) / 31536e6)), rounds: form.rounds, end: form.end })); }
    catch (e) { setError((e as Error).message); }
    finally { setBusy(""); }
  }
  ...
  return <div className="stack">
    <Card title="全球明星科技大佬 / 基金"><div className="featured-grid">{featured.map((item) => <article className="featured" key={item.id}><span className="rank">#{item.rank}</span><Star size={18} /><h3>{item.name}</h3><p>{item.fund}</p><small>{item.style}</small><Button busy={busy === item.id} onClick={() => follow(item.id)}>一键 Follow</Button></article>)}</div></Card>
...
function HoldingsPage() {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const load = () => { setBusy(true); get<Json>("/api/v1/holdings").then(setData).catch((e) => setError(e.message)).finally(() => setBusy(false)); };
  useEffect(load, []);
  async function save(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { setData(await post<Json>("/api/v1/holdings", form)); setForm({ ticker: "", name: "", quantity: 0, avg_cost: 0 }); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }
  async function remove(ticker: string) { await del(`/api/v1/holdings/${ticker}`); load(); }
  const totals = Object.entries(data.totals || {}).map(([currency, value]: [string, any]) => ({ label: `${currency} 市值`, value: fmt(value.market_value), tone: value.total_gain >= 0 ? "positive" : "negative" }));
  return <div className="stack">{totals.length > 0 && <Metrics items={totals} />}<Card title="新增 / 更新持仓"><form onSubmit={save} className="form-grid"><Field label="股票代码"><input required value={form.ticker} onChange={(e) => setForm({ ...form, ticker: e.target.value.toUpperCase() })} placeholder="NVDA" /></Field><Field label="名称"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></Field><Field label="数量"><input type="number" min="0" step="any" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: Number(e.target.value) })} /></Field><Field label="平均成本"><input type="number" min="0" step="any" value={form.avg_cost} onChange={(e) => setForm({ ...form, avg_cost: Number(e.target.value) })} /></Field><Button busy={busy} type="submit"><Plus size={15} />保存</Button></form><ErrorBox error={error} /></Card><Card title={`持仓列表 · ${data.count || 0}`} action={<Button secondary busy={busy} onClick={load}><RefreshCw size={14} />刷新行情</Button>}><div className="holding-list">{(data.holdings || []).map((item: Json) => <article className="holding" key={item.ticker}><div className="ticker-icon">{item.ticker.slice(0, 2)}</div><div><strong>{item.ticker}</strong><span>{item.name || item.status}</span></div><div><small>最新价</small><b>{fmt(item.last_price)} {item.currency}</b></div><div><small>市值</small><b>{fmt(item.market_value)}</b></div><div><small>累计盈亏</small><b className={item.total_gain >= 0 ? "positive" : "negative"}>{fmt(item.total_gain)} ({fmt(item.total_gain_pct)}%)</b></div><button className="icon-button danger" onClick={() => remove(item.ticker)} aria-label={`删除 ${item.ticker}`}><Trash2 size={16} /></button></article>)}</div>{!(data.holdings || []).length && <div className="empty">还没有持仓，可从热点、分析或 Follow 组合加入。</div>}</Card></div>;
}
...
function App() {
  const [page, setPage] = useState<Page>("vibe");
  const [menu, setMenu] = useState(false);
  const [backend, setBackend] = useState(false);
  useEffect(() => { get("/health").then(() => setBackend(true)).catch(() => setBackend(false)); }, []);
  const pages: Record<Page, ReactNode> = { vibe: <VibePage />, hotspots: <HotspotsPage />, dashboard: <DashboardPage />, analyze: <AgentReportPage kind="analyze" />, evolve: <EvolvePage />, holdings: <HoldingsPage />, backtest: <BacktestPage />, track: <AgentReportPage kind="track" />, screen: <AgentReportPage kind="screen" />, ask: <AgentReportPage kind="ask" /> };
  let lastGroup = "";
